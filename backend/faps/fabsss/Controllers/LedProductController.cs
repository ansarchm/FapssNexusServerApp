...
namespace fabsss.Controllers
{
    [ApiController]
    [Route("api/[Action]")]
    public class LedProductController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public LedProductController(IConfiguration configuration)
        {
        ...
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> GetLedProducts()
        {
            try
            {
                ...
                Console.WriteLine($"✅ Found {products.Count} LED products");
        ...
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> GetLedProductById([FromBody] JsonElement request)
        {
            try
            {
                var userId = User.FindFirst("id")?.Value;
                ...
                string query = @"SELECT * FROM tbl_product 
                                WHERE id = @id AND category = 'LED Products'";
                ...
                {
                    return NotFound(new { error = "LED product not found" });
                }
        ...
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> AddLedProduct([FromBody] Dictionary<string, object> request)
        {
            try
            {
                var userId = User.FindFirst("id")?.Value;
                ...
                Console.WriteLine($"📥 Received LED product request: {System.Text.Json.JsonSerializer.Serialize(request)}");
                ...
                string insertQuery = @"
                    ...
                    (productname, category, ptype, rate, tax, status, sequence, bonus, duration, 
                     ...
                     cardexpirydate, vipcard, poscounter, taxcategory, cardquantity, 
                     accessprofile, count) 
                    VALUES 
                ...
                var duration = GetValue<int>("Duration", 0);
                var green = GetValue<int>("Green", 0);
                var blue = GetValue<int>("Blue", 0);
                var red = GetValue<int>("Red", 0);
                var count = GetValue<int>("Count", 0);

                Console.WriteLine($"📊 Parsed LED values - Rate: {rate}, Green: {green}, Blue: {blue}, Red: {red}, Count: {count}");

                cmd.Parameters.AddWithValue("@productname", productName);
                cmd.Parameters.AddWithValue("@category", "LED Products");
                cmd.Parameters.AddWithValue("@ptype", GetValue("PType", "Product"));
                ...
                cmd.Parameters.AddWithValue("@cashbalance", cashBalance > 0 ? cashBalance : rate);
                ...
                cmd.Parameters.AddWithValue("@enableled", GetValue<int>("EnableLed", 1)); // Default to 1 for LED products
                ...
                cmd.Parameters.AddWithValue("@pricenotax", priceNoTax > 0 ? priceNoTax : rate);
                ...
                cmd.Parameters.AddWithValue("@facevalue", faceValue > 0 ? faceValue : rate);
                cmd.Parameters.AddWithValue("@sellingprice", sellingPrice > 0 ? sellingPrice : rate);
                cmd.Parameters.AddWithValue("@membership", GetValue("Membership", ""));
                cmd.Parameters.AddWithValue("@cardvalidity", GetValue<int>("CardValidity", duration));
                ...
                Console.WriteLine("✅ LED product added successfully");
                return Ok(new { message = "LED product added successfully" });
            }
            catch (MySqlException ex) when (ex.Number == 1062)
            {
                return BadRequest(new { error = "LED product already exists" });
            }
            catch (Exception ex)
            {
        ...
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> UpdateLedProduct([FromBody] LedProductModel product)
        {
            try
            {
                var userId = User.FindFirst("id")?.Value;
                ...
                cmd.Parameters.AddWithValue("@category", "LED Products");
                cmd.Parameters.AddWithValue("@ptype", product.ptype ?? "Product");
                ...
                cmd.Parameters.AddWithValue("@bonus", product.bonus ?? 0);
                cmd.Parameters.AddWithValue("@duration", product.duration ?? 0);
                cmd.Parameters.AddWithValue("@cashbalance", product.cashbalance ?? product.rate);
                ...
                cmd.Parameters.AddWithValue("@depositamount", product.depositamount ?? 0);
                cmd.Parameters.AddWithValue("@kot", product.kot);
                cmd.Parameters.AddWithValue("@favoriteflag", product.favoriteflag ?? 0);
                cmd.Parameters.AddWithValue("@customercard", product.customercard);
                cmd.Parameters.AddWithValue("@kiosk", product.kiosk ?? 0);
                cmd.Parameters.AddWithValue("@regman", product.regman ?? 0);
                cmd.Parameters.AddWithValue("@typegate", product.typegate ?? "");
                cmd.Parameters.AddWithValue("@gatevalue", product.gatevalue ?? "");
                cmd.Parameters.AddWithValue("@commonflag", product.commonflag ?? 0);
                cmd.Parameters.AddWithValue("@expiry", product.expiry ?? 0);
                cmd.Parameters.AddWithValue("@enableled", product.enableled ?? 1);
                cmd.Parameters.AddWithValue("@green", product.green ?? 0);
                cmd.Parameters.AddWithValue("@blue", product.blue ?? 0);
                cmd.Parameters.AddWithValue("@red", product.red ?? 0);
                ...
                cmd.Parameters.AddWithValue("@cardvalidity", product.cardvalidity ?? product.duration ?? 0);
                ...
                cmd.Parameters.AddWithValue("@vipcard", product.vipcard ?? 0);
                ...
                cmd.Parameters.AddWithValue("@count", product.count ?? 0);
                ...
                if (rowsAffected > 0)
                {
                    Console.WriteLine("✅ LED product updated successfully");
                    return Ok(new { message = "LED product updated successfully" });
                }
                else
                {
                    return NotFound(new { error = "LED product not found" });
        ...
        // Delete LED product (soft delete by setting status to 0)
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> DeleteLedProduct([FromBody] JsonElement request)
        {
            try
            {
                var userId = User.FindFirst("id")?.Value;
                ...
                if (rowsAffected > 0)
                {
                    return Ok(new { message = "LED product deleted successfully" });
                }
                else
                {
                    return NotFound(new { error = "LED product not found" });
