...
namespace fabsss.Controllers
{
    [ApiController]
    [Route("api/[Action]")]
    public class TimeProductController : ControllerBase
    {
        private readonly IConfiguration _configuration;

        public TimeProductController(IConfiguration configuration)
        {
            _configuration = configuration;
        }

        // Get all time products (TimeBand Categoryyy only)
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> GetTimeProducts()
        {
            try
            {
                ...
                string query = @"SELECT * FROM tbl_product 
                                WHERE category = 'TimeBand Categoryyy'
                                ORDER BY id DESC";
                ...
                Console.WriteLine($"✅ Found {products.Count} time products");
        ...
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> GetTimeProductById([FromBody] JsonElement request)
        {
            try
            {
                var userId = User.FindFirst("id")?.Value;
                ...
                string query = @"SELECT * FROM tbl_product 
                                WHERE id = @id AND category = 'TimeBand Categoryyy'";
                ...
                {
                    return NotFound(new { error = "Time product not found" });
                }
        ...
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> AddTimeProduct([FromBody] Dictionary<string, object> request)
        {
            try
            {
                var userId = User.FindFirst("id")?.Value;
                ...
                Console.WriteLine($"📊 Parsed values - Rate: {rate}, Duration: {duration}, CashBalance: {cashBalance}, Bonus: {bonus}");

                cmd.Parameters.AddWithValue("@productname", productName);
                cmd.Parameters.AddWithValue("@category", "TimeBand Categoryyy");
                cmd.Parameters.AddWithValue("@ptype", GetValue("PType", "Time"));
                ...
                Console.WriteLine("✅ Time product added successfully");
                return Ok(new { message = "Time product added successfully" });
            }
            catch (MySqlException ex) when (ex.Number == 1062)
            {
                return BadRequest(new { error = "Time product already exists" });
            }
            catch (Exception ex)
            {
        ...
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> UpdateTimeProduct([FromBody] TimeProductModel product)
        {
            try
            {
                var userId = User.FindFirst("id")?.Value;
                ...
                cmd.Parameters.AddWithValue("@category", "TimeBand Categoryyy");
                cmd.Parameters.AddWithValue("@ptype", product.ptype ?? "Time");
                ...
                cmd.Parameters.AddWithValue("@cardvalidity", product.cardvalidity.HasValue ? product.cardvalidity.Value : product.duration);
                ...
                if (rowsAffected > 0)
                {
                    Console.WriteLine("✅ Time product updated successfully");
                    return Ok(new { message = "Time product updated successfully" });
                }
                else
                {
                    return NotFound(new { error = "Time product not found" });
        ...
        // Delete time product (soft delete by setting status to 0)
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> DeleteTimeProduct([FromBody] JsonElement request)
        {
            try
            {
                var userId = User.FindFirst("id")?.Value;
                ...
                if (rowsAffected > 0)
                {
                    return Ok(new { message = "Time product deleted successfully" });
                }
                else
                {
                    return NotFound(new { error = "Time product not found" });
